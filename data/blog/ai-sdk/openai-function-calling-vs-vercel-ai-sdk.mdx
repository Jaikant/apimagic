...
faq:
  - question: What's the main difference between OpenAI's native function calling and Vercel AI SDK?
    answer: OpenAI's native API requires manual JSON schema definitions and orchestration, giving you full control but more boilerplate. Vercel AI SDK uses Zod schemas for automatic type inference and handles the function execution loop, providing better developer experience at the cost of some flexibility.
  - question: Which approach offers better type safety?
    answer: Vercel AI SDK with Zod provides superior type safety through automatic TypeScript type inference from schemas. The native OpenAI approach requires manually keeping JSON schemas and TypeScript types in sync, increasing the risk of runtime errors.
  - question: When should I use the native OpenAI API instead of AI SDK?
    answer: Choose native OpenAI API for dynamic tool generation at runtime, minimal dependencies, maximum control over the execution flow, or when working with very large numbers of tools that need database-driven definitions.
  - question: How does the AI SDK handle function execution differently?
    answer: AI SDK automatically handles the function execution loop - it calls your execute function with parsed arguments and feeds results back to the model. With native OpenAI, you must manually parse function calls, execute functions, and make follow-up API calls.
  - question: Can I use both approaches in the same application?
    answer: Yes, you can mix approaches. Some developers use AI SDK for static, well-defined tools while using native OpenAI API for dynamic tools that need runtime generation. The AI SDK also supports raw JSON schemas via jsonSchema() helper for hybrid scenarios.
  - question: What are the performance implications of each approach?
    answer: Native OpenAI has minimal overhead with just JSON processing. AI SDK adds Zod validation and schema conversion overhead, but this is typically negligible compared to the actual model API call latency.
  - question: How do I migrate from native OpenAI to AI SDK?
    answer: Replace JSON schema definitions with equivalent Zod schemas, wrap your function logic in tool() helpers with execute functions, and replace manual function call handling with AI SDK's generateText or streamText methods.
  - question: Which approach is better for large SaaS applications?
    answer: Large SaaS often benefits from native OpenAI API when tool definitions come from databases or need frequent changes. However, AI SDK works well for large codebases with many static tools, providing better maintainability through type safety.
  - question: Does AI SDK work with all OpenAI models?
    answer: AI SDK supports GPT-3.5 Turbo, GPT-4, and newer models that have function calling capabilities. It's also designed to work across multiple providers, not just OpenAI.
  - question: What happens if Zod validation fails in AI SDK?
    answer: When the model returns function arguments that don't match your Zod schema, AI SDK throws a validation error. This prevents executing functions with invalid data, unlike native OpenAI where validation is optional and developer-implemented.
---
...
**What's the main difference between OpenAI's native function calling and Vercel AI SDK?**
OpenAI's native API requires manual JSON schema definitions and orchestration, giving you full control but more boilerplate. Vercel AI SDK uses Zod schemas for automatic type inference and handles the function execution loop, providing better developer experience at the cost of some flexibility.

**Which approach offers better type safety?**
Vercel AI SDK with Zod provides superior type safety through automatic TypeScript type inference from schemas. The native OpenAI approach requires manually keeping JSON schemas and TypeScript types in sync, increasing the risk of runtime errors.

**When should I use the native OpenAI API instead of AI SDK?**
Choose native OpenAI API for dynamic tool generation at runtime, minimal dependencies, maximum control over the execution flow, or when working with very large numbers of tools that need database-driven definitions.

**How does the AI SDK handle function execution differently?**
AI SDK automatically handles the function execution loop - it calls your execute function with parsed arguments and feeds results back to the model. With native OpenAI, you must manually parse function calls, execute functions, and make follow-up API calls.

**Can I use both approaches in the same application?**
Yes, you can mix approaches. Some developers use AI SDK for static, well-defined tools while using native OpenAI API for dynamic tools that need runtime generation. The AI SDK also supports raw JSON schemas via jsonSchema() helper for hybrid scenarios.

**What are the performance implications of each approach?**
Native OpenAI has minimal overhead with just JSON processing. AI SDK adds Zod validation and schema conversion overhead, but this is typically negligible compared to the actual model API call latency.

**How do I migrate from native OpenAI to AI SDK?**
Replace JSON schema definitions with equivalent Zod schemas, wrap your function logic in tool() helpers with execute functions, and replace manual function call handling with AI SDK's generateText or streamText methods.

**Which approach is better for large SaaS applications?**
Large SaaS often benefits from native OpenAI API when tool definitions come from databases or need frequent changes. However, AI SDK works well for large codebases with many static tools, providing better maintainability through type safety.

**Does AI SDK work with all OpenAI models?**
AI SDK supports GPT-3.5 Turbo, GPT-4, and newer models that have function calling capabilities. It's also designed to work across multiple providers, not just OpenAI.

**What happens if Zod validation fails in AI SDK?**
When the model returns function arguments that don't match your Zod schema, AI SDK throws a validation error. This prevents executing functions with invalid data, unlike native OpenAI where validation is optional and developer-implemented.
