...
faq:
  - question: What is the tool_choice parameter and why is it important?
    answer: tool_choice controls when and how an LLM uses external tools. It's important because it determines whether the model can call functions ("auto"), must call functions ("required"), cannot call functions ("none"), or must call a specific function. This setting directly affects the model's behavior and reliability in different use cases.
  - question: 'When should I use tool_choice="auto" vs "required"?'
    answer: 'Use "auto" for general-purpose assistants where tools should be used only when needed, like chatbots that sometimes need external data. Use "required" when you always want external verification or grounding, such as compliance systems that must check a database for every query.'
  - question: 'What happens if I set tool_choice="none"?'
    answer: The model will not call any tools and will answer purely from its training data. This is useful for creative tasks, offline environments, or when you want to prevent external calls for security reasons. However, the model might hallucinate answers for queries requiring current data.
  - question: How do I force the model to use a specific tool?
    answer: Set tool_choice to a JSON object specifying the tool, like {"type":"function","name":"my_function"}. This forces the model to call only that specific function. Use this when your application logic has already determined which tool is needed.
  - question: What are parallel tool calls and when should I enable them?
    answer: Parallel tool calls allow the model to invoke multiple tools simultaneously in one response, reducing latency for queries requiring multiple data sources. Enable this when you have independent tools that can run concurrently, like fetching weather from multiple cities.
  - question: 'Why isn''t my model calling tools when it should in "auto" mode?'
    answer: This usually happens when the prompt lacks clear guidance. Ensure your system message explicitly mentions tool availability and when to use them. Also check that tool descriptions clearly explain their purpose and expected use cases.
  - question: How do I write effective tool descriptions?
    answer: 'Write clear, natural language descriptions that explain what the tool does and when to use it. Include context about the expected inputs and outputs. For example, "Gets current weather. Use this when user asks about weather or temperature" rather than just "Weather API".'
  - question: 'What are the risks of using tool_choice="required"?'
    answer: The model might make irrelevant or forced tool calls when no tool actually matches the query well. This can lead to nonsensical function calls or unnecessary latency. Always ensure you have appropriate tools for the expected query types.
  - question: How does tool_choice interact with parallel_tool_calls?
    answer: 'tool_choice governs which tools can be used, while parallel_tool_calls governs how many at once. For example, "auto" + parallel allows the model to call multiple tools simultaneously if needed, while "required" + parallel ensures at least one tool but possibly several concurrent calls.'
  - question: Can I change tool_choice mid-conversation?
    answer: 'Yes, you can adjust tool_choice for each API request in the same conversation. This allows dynamic control - for example, using "required" for fact-checking queries but "auto" for general conversation within the same session.'
  - question: 'What''s the difference between tool_choice="required" and forcing a specific tool?'
    answer: '"required" means the model must call at least one of the available tools (it chooses which), while forcing a specific tool means it must call that exact function. Use "required" when any tool would be helpful, and force specific tools when you know exactly which one is needed.'
  - question: How do I handle tool call failures or errors?
    answer: Always validate tool call arguments before execution, implement error handling in your tool functions, and consider providing error feedback to the model so it can adjust its approach. The prompt design should also specify how to handle missing or invalid tool inputs.
---
...
**What is the tool_choice parameter and why is it important?**
tool_choice controls when and how an LLM uses external tools. It's important because it determines whether the model can call functions ("auto"), must call functions ("required"), cannot call functions ("none"), or must call a specific function. This setting directly affects the model's behavior and reliability in different use cases.

**When should I use tool_choice="auto" vs "required"?**
Use "auto" for general-purpose assistants where tools should be used only when needed, like chatbots that sometimes need external data. Use "required" when you always want external verification or grounding, such as compliance systems that must check a database for every query.

**What happens if I set tool_choice="none"?**
The model will not call any tools and will answer purely from its training data. This is useful for creative tasks, offline environments, or when you want to prevent external calls for security reasons. However, the model might hallucinate answers for queries requiring current data.

**How do I force the model to use a specific tool?**
Set tool_choice to a JSON object specifying the tool, like `{"type":"function","name":"my_function"}`. This forces the model to call only that specific function. Use this when your application logic has already determined which tool is needed.

**What are parallel tool calls and when should I enable them?**
Parallel tool calls allow the model to invoke multiple tools simultaneously in one response, reducing latency for queries requiring multiple data sources. Enable this when you have independent tools that can run concurrently, like fetching weather from multiple cities.

**Why isn't my model calling tools when it should in "auto" mode?**
This usually happens when the prompt lacks clear guidance. Ensure your system message explicitly mentions tool availability and when to use them. Also check that tool descriptions clearly explain their purpose and expected use cases.

**How do I write effective tool descriptions?**
Write clear, natural language descriptions that explain what the tool does and when to use it. Include context about the expected inputs and outputs. For example, "Gets current weather. Use this when user asks about weather or temperature" rather than just "Weather API".

**What are the risks of using tool_choice="required"?**
The model might make irrelevant or forced tool calls when no tool actually matches the query well. This can lead to nonsensical function calls or unnecessary latency. Always ensure you have appropriate tools for the expected query types.

**How does tool_choice interact with parallel_tool_calls?**
tool_choice governs which tools can be used, while parallel_tool_calls governs how many at once. For example, "auto" + parallel allows the model to call multiple tools simultaneously if needed, while "required" + parallel ensures at least one tool but possibly several concurrent calls.

**Can I change tool_choice mid-conversation?**
Yes, you can adjust tool_choice for each API request in the same conversation. This allows dynamic control - for example, using "required" for fact-checking queries but "auto" for general conversation within the same session.

**What's the difference between tool_choice="required" and forcing a specific tool?**
"required" means the model must call at least one of the available tools (it chooses which), while forcing a specific tool means it must call that exact function. Use "required" when any tool would be helpful, and force specific tools when you know exactly which one is needed.

**How do I handle tool call failures or errors?**
Always validate tool call arguments before execution, implement error handling in your tool functions, and consider providing error feedback to the model so it can adjust its approach. The prompt design should also specify how to handle missing or invalid tool inputs.
